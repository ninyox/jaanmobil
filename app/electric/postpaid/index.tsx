...
export default function PostpaidProp() {
  ...
  const [postpaidArray, setPostpaidArray] = useState([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await Log({
        ...
        serviceType: image.service_type ? image.service_type : "",
  ...
  const fetchElectric = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const newPostpaidArray = mydata.filter((item: any) =>
          item.name.match(/POSTPAID/i),
        );
        console.log(newPostpaidArray[0]);
        setPostpaidArray(newPostpaidArray);
      }
    } catch (error) {
      openToast(" We couldn’t load electricity providers at the moment.");
    } finally {
      setIsloading(false);
    }
  };
          ...
          <ScrollView className="flex-1" horizontal={true}>
            {amountArray.map((item, index) => {
              return (
                <Text
                  key={index}
                  className=" mx-1 dark:text-white text-ten"
