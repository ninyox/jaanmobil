...
export default function ChatScreen({ submit, receivedData, optiondelete }) {
  ...
  const [visiblechats, setVisible] = useState(20);
  ...
  const start = Math.max(receivedData.length - visiblechats, 0);

  return (
    <>
      <View className="w-full bg-white h-auto flex-grow flex flex-col justify-between lol:bg-gray-900 relative scrollbar-hide">
        <ImageBackground
          imageStyle={{ opacity: 0.3, resizeMode: "repeat", height: "100%" }}
          source={deal}
          className="flex flex-col w-full flex-1 bg-[url('/chatbg.jpeg')] bg-cover"
        >
          ...
          <FlatList
            ...
            data={receivedData.slice(start, receivedData.length)}
              ...
              >
                <View className="min-w-20 w-3/5 md:w-2/5 h-auto block">
                  <TouchableOpacity
                    ...
                    className={`${
                      item.sender === userd
                        ? "bg-yellow-600 rounded-tr-none"
                        : "bg-slate-500 rounded-tl-none"
                    } min-w-20 rounded-[10px] w-auto h-auto min-h-10 px-3 py-4 my-2`}
                  >
