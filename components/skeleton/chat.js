...
const Skeleton = () => {
  ...
  return (
    <>
      <Customview>
        <View className="flex-1 w-screen h-screen justify-around flex flex-col px-3 z-10">
          <View className="skeleton-loading w-full h-10 mb-4 rounded-md" />
          {card.map((item, indexe) => (
            ...
            >
              <Animated.View className={`h-1/2 w-full flex ${typeof(item) === "string" ? "flex-row" : "flex-row-reverse"} justify-between items-center`}>
                <Animated.View
                  style={[animatedStyle]}
